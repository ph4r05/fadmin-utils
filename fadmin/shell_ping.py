# https://github.com/rogers0/shell_ping/blob/master/shell_ping.py
# https://github.com/mozillazg/justping/blob/master/LICENSE.txt
# MIT
...
def ping(target, times=4):
    ...
    # packet loss rate
    lost = re.findall(b'\d+.\d+(?=%)', text)
    if not lost:
        lost = re.findall(b'\d+(?=%)', text)
